...
class TestClient:
    ...
    def test_get_projects_success(self, client, project_data, get_projects_response):
        with patch('datagym.client.Client._request', return_value=get_projects_response), \
                patch('datagym.client.Client._response_valid', return_value=True):

            projects = client.get_projects()
            assert projects is not None
            assert len(projects) == 1

    # @patch('datagym.client.Client.requests.get')
